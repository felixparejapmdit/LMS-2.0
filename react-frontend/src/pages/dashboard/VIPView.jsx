...
export default function VIPView() {
  ...
  const getStepLabel = (name = "") => {
    ...
    if (up.includes("VEM") && !up.includes("AVEM") && !up.includes("AEVM"))
      return "VEM LETTER";
    if (up.includes("AEVM") || up.includes("AVEM")) return "AEVM LETTER";
    return name.toUpperCase();
  };

  const vipTabGradient = "linear-gradient(90deg, #4E71FF 0%, #8DD8FF 100%)";
  const vipTabActiveColor = "#4E71FF";
  const vipTabInactiveColor = "#8DD8FF";

  const getStepStyle = (name = "") => {
    const up = name.toUpperCase();
    if (up.includes("REVIEW")) return { icon: CheckCircle2 };
    if (up.includes("SIGNATURE")) return { icon: Edit2 };
    if (up.includes("VEM") && !up.includes("AVEM") && !up.includes("AEVM"))
      return { icon: Send };
    if (up.includes("AEVM") || up.includes("AVEM"))
  ...
  useEffect(() => {
    if (!selectedStepId || !user?.id) return;
    ...
    const fetchLetters = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${API_URL}/letter-assignments`, {
          params: {
        ...
        console.error(
          "Error fetching letters for step:",
          selectedStepId,
          error,
        );
        setLetters([]);
  ...
  const fetchComments = async (letterId, options = {}) => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/comments/letter/${letterId}`,
      );
  ...
  const cardBg =
    layoutStyle === "grid"
      ...
      : layoutStyle === "notion"
        ? "bg-white dark:bg-[#191919] border-gray-200 dark:border-[#333]"
      ...
      <div className={`p-6 md:p-8 rounded-3xl border ${cardBg}`}>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1
                className={`text-2xl md:text-3xl font-black tracking-tighter uppercase ${textColor}`}
              >
                LMS{" "}
      ...
      {canStepSelector && (
        <div className="overflow-x-auto pb-2 custom-scrollbar -mx-2 px-2">
          <div className="flex items-center gap-3 min-w-max">
            {steps.map((step) => {
              const stepStyle = getStepStyle(step.step_name);
              ...
              return (
                <button
                  ...
                  className={`px-5 py-3.5 rounded-[1.25rem] border transition-all flex items-center gap-3 ${
                    isSelected
                      ? "border-transparent text-white shadow-lg shadow-blue-500/20"
                      : "border-slate-200 text-slate-700 hover:border-blue-500/30 hover:text-slate-900"
                  }`}
                  style={{
                    backgroundImage: isSelected ? vipTabGradient : undefined,
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
                      isSelected
                        ...
                        : "bg-white dark:bg-[#111] text-slate-500"
                    }`}
                  >
                  ...
                  <div className="flex flex-col items-start">
                    ...
                    {step.count !== undefined && (
                      <span
                        className={`text-[9px] font-bold uppercase tracking-widest mt-0.5 ${isSelected ? "text-white/80" : "text-slate-500"}`}
                      >
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap md:whitespace-normal">
            <thead>
              <tr className="bg-slate-50/80 dark:bg-[#1A1A1A] border-b border-slate-200 dark:border-[#333]">
                ...
                <th className="px-5 py-4 w-32 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 text-center">
                  Action
                </th>
                <th className="px-5 py-4 w-1/5 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">
                  Sender
                </th>
                <th className="px-5 py-4 w-32 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">
                  Status
                </th>
                <th className="px-5 py-4 w-40 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">
                  Process
                </th>
                <th className="px-5 py-4 w-20 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 text-center">
                  Tray
                </th>
                <th className="px-5 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">
                  Topics
                </th>
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-center">
                      ...
                      (letter.attachment_id || letter.scanned_copy) ? (
                        ...
                        <div
                          className="w-10 h-10 mx-auto rounded-xl bg-slate-100 dark:bg-white/5 text-slate-300 dark:text-slate-600 flex items-center justify-center cursor-not-allowed"
                          title="No Attachment"
                        >
                          <X className="w-4 h-4" />
                        </div>
                    ...
                    <td className="px-5 py-4">
                      <p
                        className={`text-[11px] font-black uppercase tracking-tight line-clamp-2 ${letter.comments?.length > 0 ? "text-slate-400" : textColor}`}
                      >
                        {letter.sender}
                      </p>
                    </td>
                    ...
                    <td className="px-5 py-4 max-w-xl">
                      <p
                        className={`text-[11px] font-medium leading-relaxed italic line-clamp-2 ${letter.comments?.length > 0 ? "text-slate-400" : "text-slate-500 dark:text-slate-400"}`}
                      >
                        {letter.summary}
                      </p>
                    </td>
            ...
            <div className="w-full lg:w-[400px] xl:w-[450px] flex flex-col border-b lg:border-b-0 lg:border-r border-slate-100 dark:border-[#222] bg-slate-50/50 dark:bg-[#0A0A0A] shrink-0 h-1/2 lg:h-full">
              ...
              <div className="p-5 md:p-6 border-b border-slate-100 dark:border-[#222] flex items-center justify-between bg-white dark:bg-[#0c0c0c]">
                <div className="flex items-center gap-3">
                  ...
                  <h2
                    className={`text-lg font-black uppercase tracking-tighter ${textColor}`}
                  >
                    Review
                  </h2>
                </div>
                <button
                  ...
                  className="p-2 hover:bg-slate-100 dark:hover:bg-[#1A1A1A] rounded-lg transition-colors text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 md:p-6 space-y-6 custom-scrollbar">
                <div className="space-y-4">
                  <div>
                    <label className="text-[9px] font-black text-blue-500 uppercase tracking-[0.2em] mb-1.5 block">
                      Sender
                    </label>
                    <p className={`text-sm font-black uppercase ${textColor}`}>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-600 dark:text-slate-400 font-medium leading-relaxed italic bg-white dark:bg-[#111] border border-slate-100 dark:border-[#222] p-3 rounded-xl">
                ...
                <div className="space-y-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                  {isCommentsLoading ? (
                    ...
                    comments.map((comment) => (
                      <div
                        key={comment.id}
                        className="p-3.5 rounded-xl bg-white dark:bg-[#1A1A1A] border border-slate-100 dark:border-[#333] shadow-sm space-y-2 group/comment"
                      >
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              <button
                                ...
                                disabled={
                                  isSaving || !editingCommentBody.trim()
                                }
              ...
              <div className="p-6 border-t border-slate-100 dark:border-[#222] bg-white dark:bg-[#0c0c0c]">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3 pt-4 mt-2 border-t border-slate-100 dark:border-[#222]">
                    ...
                    <span className="text-[10px] font-black text-slate-400 mx-1 min-w-[40px] text-center">
            ...
            <div className="flex-1 bg-slate-100 dark:bg-black/40 relative overflow-hidden h-1/2 lg:h-full">
              {isPdfLoading ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 bg-red-50/50 dark:bg-red-900/10">
                  ...
                  <p className="font-black uppercase tracking-widest text-[10px] text-red-500">
                    {pdfError}
                  </p>
                </div>
              ) : pdfUrl ? (
      ...
      {isLogoutModalOpen && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-white dark:bg-[#111] rounded-[2.5rem] border border-gray-100 dark:border-white/10 shadow-2xl p-10 text-center animate-in zoom-in-95 duration-300">
            ...
            <h3
              className={`text-2xl font-black uppercase tracking-tighter mb-4 ${textColor}`}
            >
              Sign Out
            </h3>
