...
export default function ProcessSteps() {
    ...
    const canAdd = canField("process-steps", "add_button");
    const canEdit = canField("process-steps", "edit_button");
    const canDelete = canField("process-steps", "delete_button");
    const canSave = canField("process-steps", "save_button");
    const canRefresh = canField("process-steps", "refresh_button");
    const canViewToggle = canField("process-steps", "view_toggle");
    ...
    const fetchData = async (isRefreshing = false) => {
        ...
        try {
            ...
            const data = await processStepService.getAll(params);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (modalMode === 'create') {
                await processStepService.create({ ...formData, dept_id: finalDeptId });
            } else {
                await processStepService.update(selectedStep.id, { ...formData, dept_id: finalDeptId });
            }
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await processStepService.delete(id);
            fetchData();
            if (refreshSetupStatus) refreshSetupStatus();
        } catch (err) {
    ...
    const openCreateModal = () => {
        if (!canAdd) return;
        setModalMode("create");
        setFormData({ step_name: "", description: "", dept_id: "" });
    ...
    const renderCard = (item) => (
        <div key={item.id} className={`${cardBg} ${viewMode === 'grid' ? 'p-8 rounded-[2.5rem]' : 'p-4 rounded-2xl flex items-center justify-between'} border shadow-sm hover:shadow-xl hover:border-pink-200 dark:hover:border-pink-900/40 transition-all group cursor-pointer`}>
            <div className={viewMode === 'grid' ? "space-y-6" : "flex items-center gap-6 overflow-hidden flex-1"}>
                <div className="w-12 h-12 bg-pink-50 dark:bg-pink-900/10 rounded-full flex items-center justify-center text-pink-600 dark:text-pink-400 group-hover:scale-110 transition-transform shrink-0">
                    <GitMerge className="w-6 h-6" />
                </div>
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1 relative">
                        <div>
                            <h3 className={`font-black uppercase tracking-tight truncate ${textColor}`}>{item.step_name}</h3>
                            {isSuperAdmin && (
            ...
            {viewMode === 'list' && (
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-1">
                        {canEdit && <button onClick={(e) => { e.stopPropagation(); openEditModal(item); }} className="p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-xl transition-all text-gray-400 hover:text-pink-500"><Edit2 className="w-4 h-4" /></button>}
                ...
                <header className={`h-16 ${headerBg} border-b px-8 flex items-center justify-between sticky top-0 z-10 shrink-0`}>
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            <GitMerge className={`w-4 h-4 ${layoutStyle === 'minimalist' ? 'text-[#1A1A1B] dark:text-white' : 'text-pink-500'}`} />
                            <div>
                                <h1 className="text-[10px] font-black uppercase tracking-widest text-gray-400">Management</h1>
                                <h2 className={`text-sm font-black uppercase tracking-tight ${textColor}`}>Process Steps</h2>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        {canAdd && <button onClick={openCreateModal} className="hidden md:flex items-center gap-2 px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white text-[10px] font-black rounded-xl transition-all shadow-lg shadow-pink-500/20 uppercase tracking-widest"><Plus className="w-3 h-3" /> Add Step</button>}
                    </div>
                </header>
                    ...
                    <div className="max-w-[100vw] mx-auto">
                        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
                            <div className="flex flex-col md:flex-row md:items-center gap-6">
                                <h2 className={`text-3xl font-bold ${textColor}`}>Process Steps</h2>
                                {isSuperAdmin && (
                            ...
                            {canViewToggle && (
                                <div className="flex items-center gap-2 bg-white dark:bg-[#141414] p-1 rounded-2xl border border-gray-100 dark:border-[#222]">
                                    <button onClick={() => setViewMode("grid")} className={`p-2 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-pink-500 text-white shadow-md' : 'text-gray-400 hover:bg-slate-50 dark:hover:bg-white/5'}`}><LayoutGrid className="w-4 h-4" /></button>
                                    <button onClick={() => setViewMode("list")} className={`p-2 rounded-xl transition-all ${viewMode === 'list' ? 'bg-pink-500 text-white shadow-md' : 'text-gray-400 hover:bg-slate-50 dark:hover:bg-white/5'}`}><List className="w-4 h-4" /></button>
                                </div>
                            )}
                        </div>
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-40 gap-4"><Loader2 className="w-10 h-10 text-pink-500 animate-spin" /></div>
                        ) : (
                    ...
                    <div className={`${cardBg} w-full max-w-md rounded-[2.5rem] border shadow-2xl relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden`}>
                        <div className="p-8">
                            <div className="flex items-center justify-between mb-8">
                                <h3 className={`text-xl font-black uppercase tracking-tight ${textColor}`}>{modalMode === 'create' ? 'Create Step' : 'Edit Step'}</h3>
                                <button onClick={() => setIsModalOpen(false)}><X className="w-5 h-5 text-gray-400" /></button>
                            </div>
                            <form onSubmit={handleSubmit} className="space-y-4">
                                {error && <div className="text-red-500">{error}</div>}
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Step Name</label>
                                    <input type="text" required value={formData.step_name} onChange={e => setFormData({ ...formData, step_name: e.target.value })} className="w-full px-4 py-3 rounded-xl border bg-slate-50 dark:bg-white/5 border-gray-100 dark:border-[#333] text-sm font-bold placeholder:text-slate-300" placeholder="e.g. INCOMING, FOR REVIEW, OUTGOING..." />
                                </div>
                                <div className="space-y-1">
                                ...
                                {isSuperAdmin && (
                                    <div className="space-y-1">
                                        ...
                                        <select 
                                            ...
                                            className="w-full px-4 py-3 rounded-xl border bg-slate-50 dark:bg-white/5 border-gray-100 dark:border-[#333] text-sm font-bold appearance-none outline-none focus:ring-2 focus:ring-pink-500/20"
                                        >
                                ...
                                <div className="pt-4">
                                    {canSave && <button disabled={submitting} className="w-full py-4 bg-pink-600 hover:bg-pink-700 text-white font-black uppercase tracking-widest text-xs rounded-2xl flex items-center justify-center gap-2">{submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : "Save Step"}</button>}
                                </div>
                            </form>
                        </div>
                    </div>
