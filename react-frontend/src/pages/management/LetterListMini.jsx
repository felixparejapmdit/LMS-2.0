...
export default function LetterListMini({ deptId }) {
    ...
    if (letters.length === 0) return (
        <div className="py-10 text-center flex flex-col items-center gap-2">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">No active letters in this unit</p>
        </div>
    );
            ...
            <div className="overflow-hidden rounded-xl border border-gray-50 dark:border-white/5 bg-slate-50/50 dark:bg-black/20">
                <table className="w-full text-left">
                    <thead className="bg-slate-100 dark:bg-white/5 border-b border-gray-50 dark:border-white/5">
                        <tr>
                            <th className="px-4 py-2 text-[8px] font-black uppercase tracking-widest text-slate-500">LMS ID</th>
                            <th className="px-4 py-2 text-[8px] font-black uppercase tracking-widest text-slate-500">Sender</th>
                            <th className="px-4 py-2 text-[8px] font-black uppercase tracking-widest text-slate-500 text-right">Action</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                        {letters.map((assignment) => (
                            <tr key={assignment.id} className="hover:bg-white dark:hover:bg-white/5 transition-colors group">
                                <td className="px-4 py-2 text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-tighter">
                                    {assignment.letter?.lms_id}
                                </td>
                                <td className="px-4 py-2 text-[10px] font-bold text-slate-500 dark:text-slate-400 truncate max-w-[120px]">
                                    {assignment.letter?.sender}
                                </td>
                                <td className="px-4 py-2 text-right">
                                    <Link
                                        to={`/letter/${assignment.letter?.id}`}
