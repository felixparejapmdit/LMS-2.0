...
export default function Attachments() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (modalMode === "create") {
                ...
                await attachmentService.update(selectedAttachment.id, { ...formData, dept_id: finalDeptId });
            }
    ...
    const openCreateModal = () => {
        if (!canAdd) return;
        setModalMode("create");
        setFormData({ attachment_name: "", description: "", dept_id: "" });
                        ...
                        <div className="p-8">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Attachment Name</label>
                                    <input type="text" required value={formData.attachment_name} onChange={e => setFormData({ ...formData, attachment_name: e.target.value })} className="w-full px-4 py-3 rounded-xl border bg-slate-50 dark:bg-white/5 border-gray-100 dark:border-[#333] text-sm font-bold placeholder:text-slate-300" placeholder="e.g. USB, PHOTO, HARD DRIVE..." />
                                </div>
                                <div className="space-y-1">
                                ...
                                <div className="pt-4">
                                    {canSave && <button disabled={submitting} className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase tracking-widest text-xs rounded-2xl flex items-center justify-center gap-2">{submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : "Save Attachment"}</button>}
                                </div>
                            </form>
                        </div>
                    </div>
