...
export default function Trays() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (modalMode === 'create') {
                ...
                await trayService.updateTray(selectedTray.id, { ...formData, dept_id: finalDeptId });
            }
    ...
    const openEditModal = (tray) => {
        ...
        setFormData({
            ...
            capacity: tray.capacity || 100,
            dept_id: tray.dept_id || ""
        });
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden">
                <header className={`h-16 ${headerBg} border-b px-8 flex items-center justify-between sticky top-0 z-10 shrink-0`}>
                    ...
                    <div className="flex items-center gap-3">
                        {canRefresh && (
                            <button onClick={() => fetchTrays(true)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl transition-all">
                                <RefreshCw className={`w-4 h-4 text-gray-400 ${refreshing ? 'animate-spin' : ''}`} />
                            </button>
                        )}
                        {canAdd && (
                            <button onClick={openCreateModal} className="hidden md:flex items-center gap-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white text-[10px] font-black rounded-xl transition-all shadow-lg shadow-orange-500/20 uppercase tracking-widest">
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-12 custom-scrollbar text-sans">
                    <div className="max-w-[100vw] mx-auto">
                        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
                            ...
                            {canViewToggle && (
                                <div className="flex items-center gap-2 bg-white dark:bg-[#141414] p-1 rounded-2xl border border-gray-100 dark:border-[#222] shadow-sm">
                        ...
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-40 gap-4">
                                ...
                                <p className="text-sm text-gray-500 font-bold uppercase tracking-widest text-sans">Loading...</p>
                            </div>
                        ) : trays.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-40 gap-4 text-center bg-white dark:bg-[#141414] rounded-[2rem] border border-gray-100 dark:border-[#222]">
                                ...
                                <h3 className="text-lg font-bold text-slate-300 dark:text-slate-700 uppercase tracking-widest">No trays found</h3>
                                <p className="text-sm text-slate-400 max-w-xs">Try selecting a different filter or add a tray.</p>
                            </div>
                        ) : (
                            <div className={viewMode === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 3xl:grid-cols-6 gap-6" : "space-y-4"}>
                                {trays.map((tray) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={viewMode === 'grid' ? "space-y-6 flex-1" : "flex items-center gap-6 overflow-hidden flex-1"}>
                                                            ...
                                                            {isSuperAdmin && (
                                                                <div className="flex items-center gap-1.5 mt-1">
                                                                    ...
                                                                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                                                                        {tray.department?.dept_name || "Administrator's Default"}
                                                                    </span>
                                                                </div>
                                                            )}
                                                        </div>
                                                        <div className="relative">
                                                            ...
                                                            {isMenuOpen === tray.id && (
                                                                <div className="absolute right-0 top-full mt-2 w-32 bg-white dark:bg-[#1a1a1a] border border-gray-100 dark:border-[#333] rounded-xl shadow-xl z-20 py-1">
                                                                    {canEdit && <button onClick={() => openEditModal(tray)} className="w-full flex items-center gap-2 px-3 py-2 text-[10px] font-black uppercase text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/5"><Edit2 className="w-3 h-3" /> Edit</button>}
                                                                    {canDelete && <button onClick={() => handleDelete(tray.id)} className="w-full flex items-center gap-2 px-3 py-2 text-[10px] font-black uppercase text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10"><Trash2 className="w-3 h-3" /> Delete</button>}
                                                                </div>
                                                            )}
                                                        </div>
                                                    </div>
                                                ...
                                                {viewMode === 'grid' && (
                                                    <div className="pt-6 border-t border-gray-50 dark:border-[#222]">
                                                        ...
                                                        <div className="w-full h-1.5 bg-slate-100 dark:bg-[#222] rounded-full overflow-hidden">
                                                            <div className="h-full bg-orange-500 transition-all duration-1000" style={{ width: `${percent}%` }}></div>
                                                        </div>
                                                        <div className="flex items-center justify-between mt-3">
                                                            <div className="flex items-center gap-2">
                                                                <FileText className="w-3 h-3 text-gray-400" />
                                                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest font-sans">{count} Files</span>
                                                            </div>
                                                            {canNavigate && <button onClick={(e) => { e.stopPropagation(); navigate(`/inbox?tray=${tray.tray_no}`); }} className="text-[10px] font-black text-orange-600 uppercase tracking-widest flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">Check <ChevronRight className="w-3 h-3" /></button>}
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            {viewMode === 'list' && (
                                                <div className="flex items-center gap-4">
                                                    <div className="text-right hidden md:block shrink-0">
                                                        ...
                                                        <p className={`text-sm font-black ${count >= capacity ? 'text-red-500' : 'text-orange-500'}`}>{count} / {capacity}</p>
                                                    </div>
                                                    <div className="flex items-center gap-1">
                                                        {canEdit && <button onClick={(e) => { e.stopPropagation(); openEditModal(tray); }} className="p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-xl transition-all text-gray-400 hover:text-blue-500" title="Edit"><Edit2 className="w-4 h-4" /></button>}
                                                        {canDelete && <button onClick={(e) => { e.stopPropagation(); handleDelete(tray.id); }} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all text-gray-400 hover:text-red-500" title="Delete"><Trash2 className="w-4 h-4" /></button>}
                                                        {canNavigate && <button onClick={(e) => { e.stopPropagation(); navigate(`/inbox?tray=${tray.tray_no}`); }} className="p-2 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded-xl transition-all text-gray-300 hover:text-orange-500"><ChevronRight className="w-5 h-5" /></button>}
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                ...
                                {canAdd && (
                                    ...
                                    >
                                        ...
                                        <span className="text-xs font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest group-hover:text-orange-500 font-sans">+ New Tray</span>
                                    </button>
                                )}
                            </div>
                        )}
                                ...
                                <div className="flex items-center justify-between mb-8">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest font-sans">{trayLetters.length} Letters in this tray</p>
                                        </div>
                                    </div>
                                    <button onClick={() => { setSelectedTray(null); setTrayLetters([]); }} className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl transition-colors"><X className="w-5 h-5 text-gray-400" /></button>
                                </div>
                                        ...
                                        <table className="w-full text-left">
                                            <thead>
                                                <tr className="border-b dark:border-[#222]">
                                                    ...
                                                    <th className="py-4 px-2 text-[10px] font-black text-gray-400 uppercase tracking-widest text-sans">Sender</th>
                                                    <th className="py-4 px-2 text-[10px] font-black text-gray-400 uppercase tracking-widest text-sans">Summary</th>
                                                    <th className="py-4 px-2 text-[10px] font-black text-gray-400 uppercase tracking-widest text-sans">Status</th>
                                                ...
                                                {trayLetters.map(letter => (
                                                    <tr key={letter.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group border-b dark:border-[#1a1a1a]">
                                                        ...
                                                        <td className="py-4 px-2 text-right">
                                                            <button onClick={() => navigate(`/letter/${letter.id}`)} className="p-2 hover:bg-white dark:hover:bg-[#222] rounded-lg transition-all text-gray-300 hover:text-orange-500 opacity-0 group-hover:opacity-100"><ChevronRight className="w-4 h-4" /></button>
                                                        </td>
                                                    </tr>
                                                ))}
                                            </tbody>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className={`${cardBg} w-full max-w-md rounded-[2.5rem] border shadow-2xl relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden`}>
                        <div className="p-8">
                            <div className="flex items-center justify-between mb-8">
                                <h3 className={`text-xl font-black uppercase tracking-tight ${textColor}`}>{modalMode === 'create' ? 'Create Tray' : 'Edit Tray'}</h3>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Tray No / ID</label>
                                    <input type="text" required placeholder="e.g. A-101" value={formData.tray_no} onChange={e => setFormData({ ...formData, tray_no: e.target.value })} className="w-full px-6 py-4 rounded-2xl border bg-slate-50 dark:bg-white/5 border-gray-100 dark:border-[#333] text-sm font-bold focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all font-sans" />
                                </div>

                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Description</label>
                                    <input type="text" required placeholder="Purpose of this tray" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} className="w-full px-6 py-4 rounded-2xl border bg-slate-50 dark:bg-white/5 border-gray-100 dark:border-[#333] text-sm font-bold focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all font-sans" />
                                </div>

                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Max Capacity</label>
                                    <input type="number" required value={formData.capacity} onChange={e => setFormData({ ...formData, capacity: parseInt(e.target.value) })} className="w-full px-6 py-4 rounded-2xl border bg-slate-50 dark:bg-white/5 border-gray-100 dark:border-[#333] text-sm font-bold focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all font-sans" />
                                </div>
                                ...
                                {isSuperAdmin && (
                                    <div className="space-y-2">
                                        ...
                                        <select 
                                            ...
                                            className="w-full px-6 py-4 rounded-2xl border bg-slate-50 dark:bg-white/5 border-gray-100 dark:border-[#333] text-sm font-bold appearance-none outline-none focus:ring-2 focus:ring-orange-500 transition-all font-sans"
                                        >
                                ...
                                <div className="pt-4">
                                    {canSave && (
                                        <button disabled={submitting} className="w-full py-4 bg-orange-500 hover:bg-orange-600 text-white font-black uppercase tracking-widest text-xs rounded-2xl transition-all shadow-xl shadow-orange-500/20 flex items-center justify-center gap-2 group">
                                            {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4 group-hover:rotate-90 transition-transform" />}
                                            {modalMode === 'create' ? 'Create Tray' : 'Save Changes'}
                                        </button>
                                    )}
                                </div>
                            </form>
