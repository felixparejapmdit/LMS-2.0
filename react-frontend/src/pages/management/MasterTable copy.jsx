...
export default function MasterTable() {
  ...
  const fetchData = async (isRefreshing = false, retryCount = 0) => {
    ...
    try {
      ...
      if (response && response.data) {
        // Filter out Pending (ID 8) letters
        const data = Array.isArray(response.data) ? response.data : [];
        const filtered = data.filter(l => l.global_status !== 8);
        setLetters(filtered);
        ...
        setLetters(data.filter(l => l.global_status !== 8));
      }
  ...
  const normalizeSemicolonList = (value) => {
    const parts = splitSemicolonNames(value);
    return parts.join("; ");
  };
  ...
  const setEndorseLastToken = (fullValue, selectedName) => {
    ...
    if (parts.length === 0) parts.push(cleanName);
    else parts[parts.length - 1] = cleanName;
    const joined = parts.filter((p) => p !== "").join("; ");
    const finalValue = joined ? `${joined}; ` : "";
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm(
        "Move this letter to the Trash? You can restore it later if needed.",
      )
    )
      return;
    try {
  ...
  const handleBulkDelete = async () => {
    if (!window.confirm(`Move ${selectedIds.length} records to Trash?`)) return;
    try {
  ...
  const createEndorsement = async (letterId, name = null) => {
    const endorsedToRaw = (name || selectedLetter.endorse_to || "").trim();
    if (!endorsedToRaw) return false;
    ...
    try {
      ...
      const existingNames = new Set(
        history
          .map((e) => (e?.endorsed_to || "").toString().trim().toLowerCase())
          .filter(Boolean),
      );

      const names = splitSemicolonNames(endorsedToRaw);
      if (names.length === 0) return false;

      let createdAny = false;
      for (const endorsedTo of names) {
        const key = endorsedTo.toLowerCase();
