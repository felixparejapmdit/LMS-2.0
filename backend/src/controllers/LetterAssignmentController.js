...
class LetterAssignmentController {
  static async getAll(req, res) {
    ...
    try {
      ...
      const normalizedRole = actualRoleName
        ? actualRoleName.toString().toUpperCase().trim()
        : "";
        ...
        if (full_name) {
          ...
          nameMatches.forEach((match) => {
            ...
            visibilityClauses.push({
              "$letter.endorsed$": { [Op.like]: match },
            });
            visibilityClauses.push(
              sequelize.literal(
                `EXISTS (SELECT 1 FROM endorsements e WHERE e.letter_id = LetterAssignment.letter_id AND e.endorsed_to LIKE ${sequelize.escape(match)})`,
              ),
            );
          });
        }
      ...
      if (named_filter) {
        if (named_filter === "review") {
          ...
          where[Op.and] = [
            ...
            {
              "$step.step_name$": {
                [Op.notIn]: ["For Review", "For Signature"],
              },
            },
          ];
          ...
          where: {
            ...
            id: {
              [Op.notIn]: sequelize.literal(
                "(SELECT letter_id FROM letter_assignments WHERE letter_id IS NOT NULL)",
              ),
            },
          },
        });
      ...
      console.log(
        `[ASSIGNMENTS] Fetch complete in ${Date.now() - startTime}ms. Found ${totalCount} records.`,
      );
      res.json({
