...
class SystemPageController {
    ...
    static async create(req, res) {
        try {
            ...
            const [page] = await withSqliteBusyRetry(() => SystemPage.upsert({
                page_id,
                page_name,
                description
            }));
    ...
    static async sync(req, res) {
        try {
            ...
            // Use bulkCreate with updateOnDuplicate to handle sync in one go
            await withSqliteBusyRetry(() => SystemPage.bulkCreate(pages, {
                updateOnDuplicate: ['page_name', 'description']
            }));
            ...
            console.error('SystemPage sync failed:', error);
            res.status(500).json({ error: error.message });
        }
    }
